...
class TestWebServiceMonitor:
    ...
    def test_get_services_status(self):
        ...
        monitor.service_states = {
            "test-http": True,
            "test-port": False
        }
        monitor.down_since = {
            "test-port": datetime.now() - timedelta(minutes=30)
        }
...
class TestWebServiceMonitorRoutes:
    ...
    def test_home_with_login(self):
        ...
        with patch('uptime_monitor.dashboard.WebServiceMonitor._get_services_status') as mock_status:
